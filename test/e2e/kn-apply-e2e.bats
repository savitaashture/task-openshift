...
# Testing the kn-apply task,
@test "[e2e] kn-apply task" {
     [ -n "${E2E_KN_APPLY_PARAMS_SERVICE}" ]
     [ -n "${E2E_KN_APPLY_PARAMS_IMAGE}" ]
    
    run kubectl delete taskrun --all
    assert_success

    run tkn task start kn-apply \
        --param="SERVICE=${E2E_KN_APPLY_PARAMS_SERVICE}" \
        --param="IMAGE=${E2E_KN_APPLY_PARAMS_IMAGE}" \
        --showlog >&3
    assert_success
